...
class WaveSpeedProvider(AIProvider):
    ...
    def _poll_result(self, request_id, headers, timeout=600):
        start = time.time()
        while time.time() - start < timeout:
            time.sleep(5)
            resp = self._make_request("GET", self.RESULT_URL.format(request_id=request_id), headers=headers, timeout=60)
            data = resp.json()
    ...
    def generate(self, prompt, image_path, audio_path, resolution="720p", seed=-1, output_path=None, **kwargs) -> str:
        headers = {
